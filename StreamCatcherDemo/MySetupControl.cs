...
namespace StreamCatcherDemo
{
    public partial class MySetupControl : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            switch (flag) {
                case 0:
                    ...
                    timerDeleteSQL.Enabled = true;
                    count++;
                    break;
                case 1:
                    this.button1.Text = "Deletesql";
                    timerDeleteSQL.Enabled = false;
                    count++;
                    break;
            }
        ...
        private void m_scDeviceFormatInformation1_TextChanged(object sender, EventArgs e)
        {

        }

        private void timerDeleteSQL_Tick(object sender, EventArgs e)
        {
            string constr = @"Data Source=VPGKN7VP7JTJ06Z;Initial Catalog=a2;User ID=sa;Pwd=123456";
            SqlConnection conn = new SqlConnection(constr);
            conn.Open();
            for (int i = 1; i <= n; i++)
            {
                SqlCommand cmd = new SqlCommand("delete from Table_" + i.ToString(), conn);
                cmd.ExecuteNonQuery();
            }
